...
def fetch_zomato_image(url: str) -> str | None:
    ...
    try:
        ...
        if response.status_code == 200:
            # 1. Match any image on Zomato's CDN (handling different CDN subdomains and imagery paths)
            img_urls = re.findall(r'https://[a-z0-9]+\.zmtcdn\.com/data/[^\s"\']+\.(?:jpg|jpeg|png|webp)', response.text)
            if img_urls:
                return img_urls[0]
            # 2. General zmtcdn image search fallback
            img_urls_fallback = re.findall(r'https://[a-z0-9]+\.zmtcdn\.com/[^\s"\']+\.(?:jpg|jpeg|png|webp)', response.text)
            if img_urls_fallback:
                return img_urls_fallback[0]
    except Exception:
        pass
    return None
